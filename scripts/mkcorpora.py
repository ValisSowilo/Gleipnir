...
bar per codec, sorted best-first, with the nyx rows highlighted so where the
...
field is not ours to re-run.  Provenance is per row and per corpus:
  ...
  * nyx rows were measured here on the released 1.0.0 binary, single stream,
...
NYX = "#0969da"     # the highlight blue the Silesia charts already use
...
CORPORA = [
    ("enwik8_ranking.svg", "enwik8", 100_000_000,
     ...
     "nyx measured here on the released binary, round-trip verified. The rest "
     ...
     [("cmix v21", 14_623_723, "LTCB"), ("nncp v3.2", 14_915_298, "LTCB"),
      ...
      ("nyx -9", 18_810_676, "here"), ("lpaq1 -9", 19_755_948, "LTCB"),
      ("nyx -5", 19_660_660, "here"), ("xz -9e", 24_703_772, "LTCB"),
    ...
    ("enwik9_ranking.svg", "enwik9", 1_000_000_000,
     ...
     "nyx measured here, round-trip verified. The rest are LTCB published "
     ...
     [("nncp v3.2", 106_632_363, "LTCB"), ("cmix v21", 107_963_380, "LTCB"),
      ...
      ("nyx -9", 157_073_377, "here"), ("lpaq1 -9", 164_508_919, "LTCB"),
      ("nyx -5", 167_360_632, "here"), ("xz tuned", 197_331_816, "LTCB"),
    ...
    ("calgary_ranking.svg", "Calgary", 3_141_622,
     ...
     [("paq8px -8", 560_705, "here"), ("nyx -9", 651_967, "here"),
      ("zpaq -m5", 659_513, "here"), ("nyx -7", 661_432, "here"),
    ...
    ("canterbury_ranking.svg", "Canterbury", 2_810_784,
     ...
     "startup-dominated.",
     [("paq8px -8", 302_791, "here"), ("nyx -9", 355_766, "here"),
      ("nyx -7", 359_844, "here"), ("zpaq -m5", 362_880, "here"),
...
def ranking(path, insize, title, sub, rows):
    ...
    for k, (nm, bpc, src) in enumerate(data):
        ...
        cy = (top + bot) / 2
        c = colour(nm)
        is_nyx = nm.startswith("nyx")
        p.bar(p.px(0), top + band * 0.17, p.px(bpc) - p.px(0), band * 0.66,
              c, 1.0 if is_nyx else 0.70)
        # codec name, right-aligned in the left margin
        p.rawtext(p.ml - 10, cy + 4, nm, cls="tagb" if is_nyx else "tag",
                  anchor="end", color=c if is_nyx else None)
    ...
    p.topnote("nyx in blue; bars are bits per byte", anchor="end")
